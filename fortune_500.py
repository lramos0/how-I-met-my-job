"""
Fortune 500 name → domain pairs (curated).

The canonical browser registry lives in `lib/fortune500.js` + `data/fortune500.csv`
(full ~500 ranks from the public cmusam/fortune500 dataset). Domain overrides are
mirrored in `lib/domain-overrides.js`.
"""

companies = [
    ("Walmart", "walmart.com"),
    ("Exxon Mobil", "exxonmobil.com"),
    ("Berkshire Hathaway", "berkshirehathaway.com"),
    ("Apple", "apple.com"),
    ("UnitedHealth Group", "unitedhealthgroup.com"),
    ("McKesson", "mckesson.com"),
    ("CVS Health", "cvshealth.com"),
    ("Amazon.com", "amazon.com"),
    ("AT&T", "att.com"),
    ("General Motors", "gm.com"),
    ("Ford Motor", "ford.com"),
    ("AmerisourceBergen", "amerisourcebergen.com"),
    ("Chevron", "chevron.com"),
    ("Cardinal Health", "cardinalhealth.com"),
    ("Costco", "costco.com"),
    ("Verizon", "verizon.com"),
    ("Kroger", "kroger.com"),
    ("General Electric", "ge.com"),
    ("Walgreens Boots Alliance", "walgreensbootsalliance.com"),
    ("JPMorgan Chase", "jpmorganchase.com"),
    ("Fannie Mae", "fanniemae.com"),
    ("Alphabet", "abc.xyz"),
    ("Home Depot", "homedepot.com"),
    ("Bank of America Corp.", "bankofamerica.com"),
    ("Express Scripts Holding", "express-scripts.com"),
    ("Wells Fargo", "wellsfargo.com"),
    ("Boeing", "boeing.com"),
    ("Phillips 66", "phillips66.com"),
    ("Anthem", "antheminc.com"),
    ("Microsoft", "microsoft.com"),
    ("Valero Energy", "valero.com"),
    ("Citigroup", "citigroup.com"),
    ("Comcast", "comcastcorporation.com"),
    ("IBM", "ibm.com"),
    ("Dell Technologies", "delltechnologies.com"),
    ("State Farm Insurance Cos.", "statefarm.com"),
    ("Johnson & Johnson", "jnj.com"),
    ("Freddie Mac", "freddiemac.com"),
    ("Target", "target.com"),
    ("Lowe’s", "lowes.com"),
    ("Marathon Petroleum", "marathonpetroleum.com"),
    ("Procter & Gamble", "pg.com"),
    ("MetLife", "metlife.com"),
    ("UPS", "ups.com"),
    ("PepsiCo", "pepsico.com"),
    ("Intel", "intel.com"),
    ("DowDuPont", "dow-dupont.com"),
    ("Archer Daniels Midland", "adm.com"),
    ("Aetna", "aetna.com"),
    ("FedEx", "fedex.com"),
    ("United Technologies", "utc.com"),
    ("Prudential Financial", "prudential.com"),
    ("Albertsons Cos.", "albertsons.com"),
    ("Sysco", "sysco.com"),
    ("Disney", "disney.com"),
    ("Humana", "humana.com"),
    ("Pfizer", "pfizer.com"),
    ("HP", "hp.com"),
    ("Lockheed Martin", "lockheedmartin.com"),
    ("AIG", "aig.com"),
    ("Centene", "centene.com"),
    ("Cisco Systems", "cisco.com"),
    ("HCA Healthcare", "hcahealthcare.com"),
    ("Energy Transfer Equity", "energytransfer.com"),
    ("Caterpillar", "caterpillar.com"),
    ("Nationwide", "nationwide.com"),
    ("Morgan Stanley", "morganstanley.com"),
    ("Liberty Mutual Insurance Group", "libertymutual.com"),
    ("New York Life Insurance", "newyorklife.com"),
    ("Goldman Sachs Group", "gs.com"),
    ("American Airlines Group", "aa.com"),
    ("Best Buy", "bestbuy.com"),
    ("Cigna", "cigna.com"),
    ("Charter Communications", "charter.com"),
    ("Delta Air Lines", "delta.com"),
    ("Facebook", "facebook.com"),
    ("Honeywell International", "honeywell.com"),
    ("Merck", "merck.com"),
    ("Allstate", "allstate.com"),
    ("Tyson Foods", "tysonfoods.com"),
    ("United Continental Holdings", "united.com"),
    ("Oracle", "oracle.com"),
    ("Tech Data", "techdata.com"),
    ("TIAA", "tiaa.org"),
    ("TJX", "tjx.com"),
    ("American Express", "americanexpress.com"),
    ("Coca-Cola", "coca-colacompany.com"),
    ("Publix Super Markets", "publix.com"),
    ("Nike", "nike.com"),
    ("Andeavor", "andeavor.com"),
    ("World Fuel Services", "wfscorp.com"),
    ("Exelon", "exeloncorp.com"),
    ("Massachusetts Mutual Life Insurance", "massmutual.com"),
    ("Rite Aid", "riteaid.com"),
    ("ConocoPhillips", "conocophillips.com"),
    ("CHS", "chsinc.com"),
    ("3M", "3m.com"),
    ("Time Warner", "timewarner.com"),
    ("General Dynamics", "generaldynamics.com"),
    ("USAA", "usaa.com"),
]
