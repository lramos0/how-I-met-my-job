 ...
 * Loads Salt Technologies AI's 2025 Fortune 500 JSON when online, with a local
 * seed fallback so the forum/search UI still works during development.
 */
(function () {
  var DATA_URLS = [
    "data/fortune500.json",
  ...
  function load() {
    setCompanies(SEED);
    return fetch(DATA_URLS[0], { cache: "no-store" }).then(function(r){ if(!r.ok) throw new Error("local missing"); return r.json(); })
  ...
  function logoUrl(c, size) {
    ...
    return "https://logo.clearbit.com/" + encodeURIComponent(domain) + "?size=" + (size || 64);
  }
