...
# Load job data (use repo-relative path: web/data/job_postings.csv)
from pathlib import Path


@st.cache_data
def load_job_data():
    # Resolve path relative to this file's parent (web/)
    repo_dir = Path(__file__).resolve().parent
    data_path = repo_dir / "data" / "job_postings.csv"
    if not data_path.exists():
        # Return empty DataFrame and show an informative message in Streamlit when app runs
        try:
            st.error(f"Job postings file not found: {data_path}")
        except Exception:
            pass
        return pd.DataFrame()
