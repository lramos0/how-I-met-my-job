...
/**
 * Invokes fn(rowObj) for every data row matching countryCode (same rules as scanCsvWindow).
 * Used to shard the CSV without loading offset windows repeatedly.
 */
function forEachFilteredCsvRow(text, countryCode, fn) {
  const wantedCountry = countryCode ? String(countryCode).trim().toUpperCase() : "";
  let headers = null;
  let countryIdx = -1;
  let totalCsvRows = 0;
  ...
  return { csvRowsTotal: totalCsvRows };
}
